...
public class StreamsPractiseQuestions {
    ...
    public static void main(String[] args) {
        ...
        Order1 o1=Order1.builder().id(201l).orderDate(LocalDate.of(2021,02,15)).deliveryDate(LocalDate.now().minusDays(3l))
                .status("Pending")
                .products(prdSet1)
                .customer(c1)
                .build();
        Order1 o2=Order1.builder().id(202l).orderDate(LocalDate.now().minusDays(2l)).deliveryDate(LocalDate.now().minusDays(2l))
                .status("Pending")
                .products(prdSet2)
                .customer(c2)
                .build();
        Order1 o3=Order1.builder().id(203l).orderDate(LocalDate.of(2021,02,28)).deliveryDate(LocalDate.now().minusDays(1l))
                .status("Pending")
                .customer(c3)
                .products(prdSet3)
                .build();
        Order1 o4=Order1.builder().id(204l).orderDate(LocalDate.now().minusDays(4l)).deliveryDate(LocalDate.now().minusDays(4l))
                .status("Pending")
                .customer(c4)
                .products(prdSet4)
                .build();
        ...
        System.out.println(orderLst.stream().sorted(Comparator.comparing(Order1::getOrderDate).reversed())
                .limit(3)
                .collect(Collectors.toList()));
        ...
        System.out.println(orderLst.stream().filter(order1 -> order1.getOrderDate().compareTo(LocalDate.of(2021,03,15))==0)
                .peek(order1 -> System.out.println(order1))
                .flatMap(order1 -> order1.getProducts().stream())
                .collect(Collectors.toList()));

        /** Exercise 8 — Calculate total lump sum of all orders placed in Feb 2021*/
        System.out.println(orderLst.stream().filter(order1 -> order1.getOrderDate().compareTo(LocalDate.of(2021,02,01))>=0)
        ...
        DoubleSummaryStatistics summaryStats=prdLst.stream().filter(product -> "Books".equals(product.getCategory()))
                .mapToDouble(Product::getPrice)
                .summaryStatistics();
        ...
        System.out.println(orderLst.stream().collect(Collectors.toMap(Function.identity(),
        ...
        System.out.println(prdLst.stream().collect(Collectors.groupingBy(Product::getCategory,
